...
\caption{On the left, an instance of the \coverage problem, and on the right a covering assignment for that instance.}
...
\caption{On the left, an instance of the \connectivity problem with two terminal groups, and on the right a connecting assignment for that instance.}
\label{example_connecting}
\end{figure}

\subsection{Related Work}

In the following, let $k$ denote the number of different interfaces across the network, $n$ the number of vertices in the input graph, $m$ the number of edges, $\Delta$ the maximum degree, $c_{\max}$ the maximum cost and $c_{\min}$ the minimum cost of an interface. In the \emph{homogeneous} setting, the costs only depend on the interface while in the \emph{heterogeneous} case they depend on the interface and on the vertex. 
...
A simpler deterministic rounding of the same LP gives a $k$-approximation, generalizing the $1+(k-2) \frac{c_{\max}}{2 c_{\min}}$-approximation for homogeneous min-max costs\cite{MinimizeTheMaximumDutyInMultiInterfaceNetworks}.

For the \connectivity problem, we extend the LP by adding flow-based cut constraints encoding group connectivity. Our rounding procedure is similar to the one for the \coverage problem, however, this time the process is iterative and the algorithm proceeds until all terminal groups are connected. Each iteration is randomized and we show that in expectation, in each such round the algorithm covers a significant fraction of edges of some graph which connects all group terminals. This yields an $O\br{\log^2 m}$-approximation algorithm, which is the first non-trivial approximation algorithm for this problem.

The rest of the paper is structured as follows:
In Section \ref{sec:preliminaries}, we give notations, define the problems formally, and recall the elements of probability theory useful for the analysis of our algorithms. We also describe the preprocessing used on instances of the \coverage and \connectivity problems.
In Section \ref{sec:coverage}, we present the $O\br{\log m}$-approximation and the $k$-approximation algorithms for the \coverage problem.
In Section \ref{sec:connectivity}, we present the $O\br{\log^2 m}$-approximation algorithm for the \connectivity problem.
