...
\begin{align*}
    \Pr\bigl\sbr{\exists\, S\subset V,\; S\neq\emptyset,\, S\neq V\colon \spr{\delta_H\br{S}}=0\bigr}
...
By $X_{v,i}^j$ denote the indicator that is true iff $i\in \lambda_A^j$. Then, $\COST\br{v, \lambda_A^j} = \sum_{i\in\lambda\br{v}}c\br{i,v} \cdot X_{v,i}^j $. We have $\mathbb{E}\sbr{X_{v,i}^j} = 5\ln m\cdot x_{i,v}$, hence $\mathbb{E}\Big\sbr{\COST\br{v,\lambda_A^j}\Big} = 5\ln m\cdot \sum_{i\in\lambda\br{v}}c\br{i,v} \cdot x_{i,v}$. Applying the Chernoff bound with $\delta=2$ gives:
\begin{align*}
