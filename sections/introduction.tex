...
\caption{On the left, an instance of the \coverage problem, and on the right a covering assignment for that instance (if an edge is covered by several interfaces, only one is represented).}
\label{fig:example_covering}
\end{figure}
...
The algorithmic study of problems in multi-interface networks started with the problem of covering a multi-interface networks, introduced in Caporuscio et al. \cite{EnergeticPerformanceOfServiceOrientedMultiRadioNetworks}. 
...
The min-sum, unit cost variant is APX-hard for any constant $k \geq 3$, and is hard to approximate within a $O\br{\ln k}$ factor. On the positive side, it has a $\min \brc{k-1, \sqrt{n} \br{1+\ln n}}$-approximation \cite{CostMinimizationInWirelessNetworks}.

The min-max variant with uniform costs is \NPhard for any constant $\Delta \geq 5$, $k \geq 16$, and it is not approximable within a $\eta \ln \Delta$ factor for some constant $\eta$ even on trees. On the positive side, with homogeneous costs it admits a $\br{\ln \Delta +1 +b \cdot \min \brc{c_{\max}, \ln \Delta +1}}$-approximation, where $b$ is a parameter that depends on the structural properties of the input graph and is bounded by many other graph structural parameters such as treewidth, arboricity or maximum degree. It also has a $(1+\br{k-2} \frac{c_{\max}}{2 c_{\min}})$-approximation and a $(\frac{\Delta}{2} \frac{c_{\max}}{c_{\min}})$-approximation \cite{MinimizeTheMaximumDutyInMultiInterfaceNetworks}. 
...
We study the approximability of the \coverage and \connectivity problems minimizing the max cost under heterogeneous cost functions. We give a set-cover-style LP relaxation for the \coverage problem and show that treating the resulting fractional solution as a probability distribution and applying a randomized rounding procedure yields an $O\br{\log m}$-approximation. This is the first logarithmic approximation for this problem with heterogeneous costs, and it matches the previously known lower bound of $\Omega\br{\log n}$ for stars with unit costs \cite{MinimizeTheMaximumDutyInMultiInterfaceNetworks}. It also improves the previously best known $O\br{\ln \Delta + b \cdot \min\brc{\ln \Delta, c_{\max}}}$-approximation (for some graph input parameter $b$), limited to the homogeneous case \cite{MinimizeTheMaximumDutyInMultiInterfaceNetworks}. 
A simpler deterministic rounding of this LP gives a $k$-approximation, asymptotically generalizing the $1+(k-2) \frac{c_{\max}}{2 c_{\min}}$-approximation for the min-max variant with homogeneous costs\cite{MinimizeTheMaximumDutyInMultiInterfaceNetworks}.
...
In \Cref{sec:coverage}, we present the $O\br{\log m}$-approximation and the $k$-approximation algorithms for the \coverage problem.
In \Cref{sec:connectivity}, we present the $O\br{\log^2 m}$-approximation algorithm for the \connectivity problem.
