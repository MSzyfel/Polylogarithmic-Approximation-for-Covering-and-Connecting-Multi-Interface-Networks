% Connectivity problem illustration - identical topology to problem-illustration.tex.
% Left: input instance with available interfaces.
% Right: one connecting assignment where all vertices are connected.
...
\vspace{1mm}
{\scriptsize Each vertex shows the set of interfaces available}
\end{minipage}
\hfill
\begin{minipage}[t]{0.49\linewidth}
\centering
\begin{tikzpicture}[scale=0.80]
    ...
    % non-activated edges (thin, faded)
    ...
    % activated edges (thick, colored by activated interface)
...
\vspace{1mm}
{\scriptsize Bold edges are all activated (covered) edges; colors indicate active interfaces}
\end{minipage}
